...
//		 System.exit(0);
	...
	public List<Authors> getAuthors() throws InterruptedException, ExecutionException, JsonMappingException, JsonProcessingException {
		ObjectMapper jacksonObjMapper = new ObjectMapper();
		System.out.println("Inside api get data");
		List<Authors> list=new ArrayList();
		
		HttpRequest request = HttpRequest.newBuilder().GET()
		          .uri(URI.create("http://localhost:8081/Authors"))
		           .build();
		System.out.println(request);
		 String s= client.sendAsync(request,BodyHandlers.ofString()) 
		          .thenApply(HttpResponse::body).get();
		 ...
		 for (JsonNode jsonNode : new_json) {
			 ...
			 author.setName(jsonNode.get("id").asText());
